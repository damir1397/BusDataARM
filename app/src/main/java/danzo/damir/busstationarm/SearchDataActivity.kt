...
class SearchDataActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (cursor.moveToFirst()) {
            val nameStop: Int = cursor.getColumnIndex(DataBase.ROUTE1)
            val route: Int = cursor.getColumnIndex(DataBase.ROUTE2)
            ...
            do {
                route1List.add(cursor.getInt(nameStop).toString())
                route2List.add(cursor.getString(route).toString())
...
//                "SELECT * FROM ${DataBase.TABLE_BUS} WHERE ${DataBase.ROUTE1} ='${route1.text}' and ${DataBase.ROUTE2} ='${route2.text}'and ${DataBase.TICKET_PRICE} ='${prise.text}'and ${DataBase.MOVEMENT_TIME_TO_STOP} ='${time.text}' ",
//                null
//            )
